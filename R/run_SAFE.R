...
run_SAFE <- function(cluster_results, k_min = 2, k_max = NULL, MCLA = TRUE, HGPA = FALSE, CSPA = FALSE, cspc_cell_max = NULL, SEED = 1){
  ...
  sgraph <- function(k, dataname){
    ...
    command <- NULL
    if (last_char < 2){
      command <- paste(paste0(program.path, '/gpmetis -seed='), SEED, ' ', dataname, ' ', k, sep = '')
